...
#if defined(DISPLAY_1_8)
  digitalWrite(BACKLIGHT_PIN, level > 0 ? LOW : HIGH);  // Active low backlight
#else
...
void initDisplay() {
  ...
  tft.setTextFont(3);
  tft.drawString("BambuHelper", SCREEN_W / 2, SCREEN_H / 2 - 24);
  tft.setTextFont(2);
  tft.setTextColor(CLR_TEXT_DIM, CLR_BG);
  tft.drawString("Printer Monitor", SCREEN_W / 2, SCREEN_H / 2 - 2);
  tft.setTextFont(1);
  tft.drawString(FW_VERSION, SCREEN_W / 2, SCREEN_H / 2 + 16);
}
...
static void drawConnectingWiFi() {
  ...
  tft.setTextFont(1);
  tft.setTextColor(CLR_TEXT, CLR_BG);
  tft.drawString("Connecting to WiFi", SCREEN_W / 2, SCREEN_H / 2 - 24);

  int16_t tw = tft.textWidth("Connecting to WiFi");
  drawAnimDots(tft, SCREEN_W / 2 + tw / 2, SCREEN_H / 2 - 30, CLR_TEXT);

  // Slide bar
  const int16_t barW = 96;
  const int16_t barH = 6;
  drawSlideBar(tft, (SCREEN_W - barW) / 2, SCREEN_H / 2 + 12,
...
static void drawWiFiConnected() {
  ...
  tft.fillCircle(cx, cy, 18, CLR_GREEN);
  // Draw thick tick mark (2px wide)
  for (int i = -1; i <= 1; i++) {
  ...
  tft.setTextColor(CLR_GREEN, CLR_BG);
  tft.setTextFont(3);
  tft.drawString("WiFi Connected", SCREEN_W / 2, SCREEN_H / 2 + 8);

  tft.setTextColor(CLR_TEXT, CLR_BG);
  tft.setTextFont(1);
  tft.drawString(WiFi.localIP().toString().c_str(), SCREEN_W / 2, SCREEN_H / 2 + 28);
}
...
static void drawOtaUpdate() {
  ...
  // Title
  tft.setTextFont(3);
  tft.drawString("Updating", SCREEN_W / 2, SCREEN_H / 2 - 50);
  tft.setTextFont(2);
  tft.setTextColor(CLR_TEXT_DIM, CLR_BG);
  tft.drawString("BambuHelper firmware", SCREEN_W / 2, SCREEN_H / 2 - 26);

  // Progress bar
  int pct = getOtaAutoProgress();
  const int16_t barX = 16, barY = SCREEN_H / 2 - 8;
  const int16_t barW = SCREEN_W - 32, barH = 10;
...
static void drawPrinting() {
  ...
  if (forceRedraw || stateChanged) {
    ...
    // Current printing state below the name
    ...
    tft.drawString(s.gcodeState, LY_HDR_NAME_X, LY_HDR_STATE_Y);

    // Progress percentage on the right side
    tft.setTextDatum(MR_DATUM);
    tft.setTextFont(2);
    tft.setTextColor(CLR_TEXT_DIM, hdrBg);
    char hdrProg[8];
    snprintf(hdrProg, sizeof(hdrProg), "%d%%", s.progress);
    int16_t progRightX = SCREEN_W - LY_HDR_BADGE_RX;
    int16_t progLeftX = progRightX - tft.textWidth(hdrProg) - 16;
    tft.fillCircle(progLeftX, LY_HDR_CY, 4, badgeColor);
    tft.drawString(hdrProg, progRightX, LY_HDR_CY);
  ...
  {
    static unsigned long lastGaugeRotateMs = 0;
    static uint8_t currentGaugePage = 0;
    const bool rotateGauges = (SCREEN_W <= 128);
    const uint8_t visibleGaugeCount = 3;
    const uint8_t gaugePageCount = (GAUGE_SLOT_COUNT + visibleGaugeCount - 1) / visibleGaugeCount;
    bool pageChanged = false;
    if (rotateGauges && millis() - lastGaugeRotateMs > 3500) {
      lastGaugeRotateMs = millis();
      currentGaugePage = (currentGaugePage + 1) % gaugePageCount;
      pageChanged = true;
    }

    const int16_t pageX[3] = { 26, 102, 64 };
    const int16_t pageY[3] = { 48, 48, 100 };
    const int16_t pageR = rotateGauges ? 22 : gR;
    const int16_t pageT = gT;

    if (rotateGauges && (forceRedraw || pageChanged)) {
      tft.fillRect(0, LY_HDR_Y + LY_HDR_H, SCREEN_W,
                   eff_etaY - (LY_HDR_Y + LY_HDR_H), CLR_BG);
    }
    ...
    for (uint8_t si = 0; si < GAUGE_SLOT_COUNT; si++) {
      ...
      const bool visibleThisPage = !rotateGauges ||
        (si >= currentGaugePage * visibleGaugeCount &&
         si < currentGaugePage * visibleGaugeCount + visibleGaugeCount);
      if (!visibleThisPage) {
        continue;
      }

      uint8_t pageIndex = si % visibleGaugeCount;
      int16_t cx = rotateGauges ? pageX[pageIndex] : (si < 3 ? LY_COL1 + si * (LY_COL2 - LY_COL1) : LY_COL1 + (si - 3) * (LY_COL2 - LY_COL1));
      int16_t cy = rotateGauges ? pageY[pageIndex] : (si < 3 ? LY_ROW1 : LY_ROW2);
      int16_t gaugeR = rotateGauges ? pageR : gR;
      int16_t gaugeT = rotateGauges ? pageT : gT;

      // Per-type change detection
      bool needDraw = forceRedraw || typeChanged || pageChanged;
      if (!needDraw) {
        switch (gt) {
          ...
          case GAUGE_CLOCK:       needDraw = true; break;
          default: break;
        }
      }
      if (!needDraw) continue;

      bool fr = forceRedraw || typeChanged || pageChanged;

      switch (gt) {
        case GAUGE_PROGRESS:
          drawProgressArc(tft, cx, cy, gaugeR, gaugeT, s.progress, prevState.progress, s.remainingMinutes, fr);
          break;
        case GAUGE_NOZZLE:
          drawTempGauge(tft, cx, cy, gaugeR, s.nozzleTemp, s.nozzleTarget, 300.0f,
                        dispSettings.nozzle.arc, nozzleLabel(s), nullptr, fr,
                        &dispSettings.nozzle, smoothNozzleTemp);
          break;
        case GAUGE_BED:
          drawTempGauge(tft, cx, cy, gaugeR, s.bedTemp, s.bedTarget, 120.0f,
                        dispSettings.bed.arc, "Bed", nullptr, fr,
                        &dispSettings.bed, smoothBedTemp);
          break;
        case GAUGE_PART_FAN:
          drawFanGauge(tft, cx, cy, gaugeR, s.coolingFanPct, dispSettings.partFan.arc, "Part", fr,
                       &dispSettings.partFan, smoothPartFan);
          break;
        case GAUGE_AUX_FAN:
          drawFanGauge(tft, cx, cy, gaugeR, s.auxFanPct, dispSettings.auxFan.arc, "Aux", fr,
                       &dispSettings.auxFan, smoothAuxFan);
          break;
        case GAUGE_CHAMBER_FAN:
          drawFanGauge(tft, cx, cy, gaugeR, s.chamberFanPct, dispSettings.chamberFan.arc, "Chamber", fr,
                       &dispSettings.chamberFan, smoothChamberFan);
          break;
        case GAUGE_CHAMBER_TEMP:
          drawTempGauge(tft, cx, cy, gaugeR, s.chamberTemp, 0.0f, 60.0f,
                        dispSettings.chamberTemp.arc, "Chamber", nullptr, fr,
                        &dispSettings.chamberTemp, smoothChamberTemp);
          break;
        case GAUGE_HEATBREAK:
          drawFanGauge(tft, cx, cy, gaugeR, s.heatbreakFanPct, dispSettings.heatbreak.arc, "HBreak", fr,
                       &dispSettings.heatbreak, smoothHeatbreakFan);
          break;
        case GAUGE_CLOCK:
          drawClockWidget(tft, cx, cy, gaugeR, gaugeT, fr);
          break;
        case GAUGE_EMPTY:
        default:
          if (fr) tft.fillCircle(cx, cy, gaugeR + 2, dispSettings.bgColor);
          break;
      }
    }
  }
  ...
  if (etaChanged || stateChanged) {
    ...
    int16_t etaBarX = (SCREEN_W - LY_ETA_BAR_W) / 2;
    int16_t etaBarY = LY_ETA_BAR_Y;
    int16_t etaBarW = LY_ETA_BAR_W;
    int16_t etaBarH = LY_ETA_BAR_H;
    ...
    if (strcmp(s.gcodeState, "PAUSE") == 0) {
      ...
      if (ntpSynced) {
        ...
        if (!netSettings.use24h) {
          ...
          etaH = etaH % 12;
          if (etaH == 0) etaH = 12;
          snprintf(etaTimeBuf, sizeof(etaTimeBuf), "%d:%02d%s", etaH, etaTm.tm_min, ampm);
        } else {
          snprintf(etaTimeBuf, sizeof(etaTimeBuf), "%02d:%02d", etaH, etaTm.tm_min);
        }

        tft.setTextFont(3);
        tft.setTextColor(CLR_GREEN, CLR_BG);
        tft.drawString(etaTimeBuf, SCREEN_W / 2, eff_etaTextY - 2);

        tft.setTextFont(1);
        tft.setTextColor(CLR_TEXT_DIM, CLR_BG);
        tft.drawString("ETA", SCREEN_W / 2, eff_etaTextY + 8);
      } else {
        char remBuf[24];
        ...
        tft.setTextFont(3);
        tft.setTextColor(CLR_TEXT, CLR_BG);
        tft.drawString(remBuf, SCREEN_W / 2, eff_etaTextY - 2);

        tft.setTextFont(1);
        tft.setTextColor(CLR_TEXT_DIM, CLR_BG);
        tft.drawString("Remain", SCREEN_W / 2, eff_etaTextY + 8);
      }

      uint16_t trackColor = dispSettings.trackColor;
      tft.fillRect(etaBarX, etaBarY, etaBarW, etaBarH, trackColor);
      int16_t fillW = (s.progress * etaBarW) / 100;
      if (fillW < 1) fillW = 1;
      tft.fillRect(etaBarX, etaBarY, fillW, etaBarH, dispSettings.progress.arc);
    } else {
      tft.setTextFont(3);
      tft.setTextColor(CLR_TEXT_DIM, CLR_BG);
      tft.drawString("---", SCREEN_W / 2, eff_etaTextY - 2);
  ...
  if (bottomChanged) {
    ...
    if (showPowerNow) {
      drawIcon16(tft, SCREEN_W * 2 / 3 - 20, eff_botCY - 8, icon_lightning, CLR_YELLOW);
      char wBuf[8];
      snprintf(wBuf, sizeof(wBuf), "%.0fW", tasmotaGetWatts());
      tft.setTextDatum(ML_DATUM);
      tft.setTextColor(CLR_TEXT_DIM, CLR_BG);
      tft.drawString(wBuf, SCREEN_W * 2 / 3 - 2, eff_botCY);
    } else {
      tft.setTextDatum(MC_DATUM);
      tft.setTextColor(CLR_TEXT_DIM, CLR_BG);
      char layerBuf[20];
      snprintf(layerBuf, sizeof(layerBuf), "L%d/%d", s.layerNum, s.totalLayers);
      tft.drawString(layerBuf, SCREEN_W * 5 / 8, eff_botCY);
    }
...
static void drawFinished() {
  ...
  if (forceRedraw) {
    ...
    tft.drawString("100%", scrW / 2, LY_HDR_Y + 10);
  }
  ...
  if (tempChanged) {
    drawTempGauge(tft, gaugeLeft, gaugeY, gR,
                  s.nozzleTemp, s.nozzleTarget, 300.0f,
                  dispSettings.nozzle.arc, nozzleLabel(s), nullptr, forceRedraw,
                  &dispSettings.nozzle, smoothNozzleTemp, true);

    drawTempGauge(tft, gaugeRight, gaugeY, gR,
                  s.bedTemp, s.bedTarget, 120.0f,
                  dispSettings.bed.arc, "Bed", nullptr, forceRedraw,
                  &dispSettings.bed, smoothBedTemp, true);
  }
